import SheetConnection, { AbstractModel, column, worksheet, Config } from '../src';

@worksheet(0)
class User extends AbstractModel {
  @column
  name: string;
  @column
  email: string;
}

let config = {
  migrate: 'drop',
  spreadsheetId: '1Vo30hV4PrtjneT5qOGDEQ3Lm2xK89yo7Fa7EFAUma60',
  authClient: SheetConnection.getAuthClient(
    'tester@testingorm.iam.gserviceaccount.com',
    '-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n',
    'c010442202e3b0abe0a2ea99d4cfcda7f3a96e09',
  ),
} as Config;

SheetConnection.connect(config).then(async (connection: SheetConnection) => {
  let user = new User();
  user.name = 'Mario Luigi';
  user.email = 'mario@luigi.com';

  await connection.setInfo(user);
});
